...
def _rank_log_groups(log_groups: list[dict]) -> list[dict]:
    ...
    Ranking: groups containing 'prod' > others > groups containing 'staging'/'uat'/'dev'/'test'.
    """
    def _score(lg: dict) -> int:
        name = lg.get("log_group_name", "").lower()
        if "prod" in name:
            return 0
        if any(env in name for env in ("staging", "stg", "uat", "dev", "test", "sandbox")):
            return 2
        return 1
...
def _get_resource_explorer_client(region: str = None):
    ...
    Uses cross-account credentials from config if available,
    otherwise falls back to Lambda execution role.
    """
    import boto3
    ...
    # PRIORITY 1: Use cross-account credentials for CloudWatch access
    cloudwatch_config = config.cloudwatch_config
    if cloudwatch_config.get("access_key_id") and cloudwatch_config.get("secret_access_key"):
        logger.info("Using cross-account credentials for Resource Explorer access")
        session_kwargs = {
            "aws_access_key_id": cloudwatch_config["access_key_id"],
            "aws_secret_access_key": cloudwatch_config["secret_access_key"],
        }
        if cloudwatch_config.get("session_token"):
            session_kwargs["aws_session_token"] = cloudwatch_config["session_token"]
        if region:
            session_kwargs["region_name"] = region
        elif cloudwatch_config.get("region"):
            session_kwargs["region_name"] = cloudwatch_config["region"]
        
        session = boto3.Session(**session_kwargs)
        return session.client("resource-explorer-2")
    
    # PRIORITY 2: Fall back to Lambda execution role (default)
    logger.info("Using Lambda execution role for Resource Explorer access")
    ...
    try:
        ...
        for priority, query in enumerate(queries, start=1):
            ...
            if matches:
                ...
                logger.info(
                    "discover_log_group: found %d candidates, ranked results: %s",
                    len(ranked),
                    [lg["log_group_name"] for lg in ranked]
                )
                ...
                logger.info(
                    "discover_log_group: selected '%s' (prod prioritized) via query '%s' (priority %d)",
